...
BACKUP="/root/era64i_runner_before_block_receipt_fix_v5_$(date -u +%Y%m%dT%H%M%SZ).sh"
...
for old,new in replacements:
    ...
    if count!=1:
        raise SystemExit(f'ERA64I_BLOCK_RECEIPT_V5_PATTERN_COUNT_INVALID:{count}:{old[:100]!r}')
...
if start<0 or end<0 or end<=start:
    raise SystemExit(f'ERA64I_BLOCK_RECEIPT_V5_FUNCTION_BOUNDARY_INVALID:{start}:{end}')
...
print('ERA64I_BLOCK_RECEIPT_SAMPLING_FIX_V5=APPLIED')
PY
...
assert 'eth_getLogs' not in text
...
echo "ERA64I_BLOCK_RECEIPT_SAMPLING_FIX_V5=VERIFIED"
