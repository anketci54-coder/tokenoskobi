...
def run(args: list[str], timeout: int = 60, check: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    if check and completed.returncode != 0:
        raise RuntimeError(
            'COMMAND_FAILED:' + ' '.join(args) + ':rc=' + str(completed.returncode)
...
def db_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        max_times: dict[str, Any] = {}
        for table in TABLES:
            if table not in existing:
                continue
            columns = [
                str(row[1])
                for row in connection.execute('PRAGMA table_info(' + q(table) + ')').fetchall()
            ]
            ...
            if timestamp_column:
                max_times[table] = connection.execute(
                    'SELECT MAX(' + q(timestamp_column) + ') FROM ' + q(table)
                ).fetchone()[0]
        return {
...
def systemd_snapshot() -> dict[str, Any]:
    ...
    for item in [service, timer]:
        ...
        item['FragmentSHA256'] = sha256_file(fragment) if fragment.is_file() else None
...
def natural_cycle_evidence(integration_time: datetime, journal_text: str, systemd: dict[str, Any]) -> dict[str, Any]:
    service = systemd['service']
    exit_time = parse_systemd_timestamp(service.get('ExecMainExitTimestamp', ''))
    marker_hot = 'OK_NEWS_HOT_INGRESS_BOUNDED_RUNTIME_REFRESH_V1' in journal_text
    marker_wrapper = (
    ...
    exit_after_binding = bool(exit_time and exit_time > integration_time)
    successful_exit = (
    ...
    return {
        'observed': bool(marker_hot and marker_wrapper and exit_after_binding and successful_exit),
        ...
        'service_exit_time_utc': exit_time.isoformat() if exit_time else None,
    }


def wait_for_natural_cycle(integration_time_text: str, max_wait_seconds: int) -> dict[str, Any]:
    ...
    while True:
        ...
        evidence = natural_cycle_evidence(integration_time, journal, systemd)
        last = {
            'attempts': attempts,
            'elapsed_seconds': int(time.monotonic() - started_monotonic),
            'systemd': systemd,
            'evidence': evidence,
            'journal_line_count': len([line for line in journal.splitlines() if line.strip()]),
            'journal_tail': '\n'.join(journal.splitlines()[-120:]),
        }
        ...
        if attempts == 1 or attempts % 6 == 0:
            print(
                'OBSERVATION_PENDING elapsed_seconds=' + str(int(elapsed))
                + ' timer_last_trigger=' + str(systemd['timer'].get('LastTriggerUSec')),
                flush=True,
            )
        time.sleep(10)


def validate_authority(authority: Any, prefix: str, failures: list[str]) -> None:
...
def main() -> int:
    ...
    expected_head = os.environ.get('POST_ERA54_OBSERVATION_EXPECTED_HEAD', '').strip()
    if not expected_head:
        raise RuntimeError('POST_ERA54_OBSERVATION_EXPECTED_HEAD_REQUIRED')
    ...
    if current_head != expected_head:
        raise RuntimeError('HEAD_MISMATCH:expected=' + expected_head + ':actual=' + current_head)
    if branch != 'main':
        raise RuntimeError('BRANCH_NOT_MAIN:' + branch)
    if git_output('status', '--porcelain=v1'):
        raise RuntimeError('WORKTREE_NOT_CLEAN_AT_OBSERVATION_START')

    integration = load_json(INTEGRATION)
    if integration.get('decision') != 'OK_POST_ERA54_HOT_INGRESS_BOUNDED_RUNTIME_INTEGRATION_NOAPI':
    ...
    integration_time_text = str(integration.get('generated_at_utc') or '')
    integration_time = parse_dt(integration_time_text)
    integration_counts = integration.get('result', {}).get('db_after', {}).get('counts', {})

    tracked_dynamic = [
        line
        for line in git_output('ls-files', '--', *DYNAMIC_PATHS).splitlines()
        if line.strip()
    ]
    if tracked_dynamic:
        raise RuntimeError('DYNAMIC_OUTPUTS_STILL_TRACKED:' + ','.join(tracked_dynamic))
    missing_dynamic = [path for path in DYNAMIC_PATHS if not (ROOT / path).exists()]
    if missing_dynamic:
        raise RuntimeError('DYNAMIC_LOCAL_FILES_MISSING:' + ','.join(missing_dynamic))
    not_ignored = [
        path
    ...
    if not_ignored:
        raise RuntimeError('DYNAMIC_OUTPUTS_NOT_IGNORED:' + ','.join(not_ignored))

    systemd_before = systemd_snapshot()
    service_fragment_sha_before = systemd_before['service'].get('FragmentSHA256')
    timer_fragment_sha_before = systemd_before['timer'].get('FragmentSHA256')
    db_before = db_snapshot()
    market_before = jsonl_stats(MARKET_JSONL)
    adversarial_before = jsonl_stats(ADVERSARIAL_JSONL)

    observation = wait_for_natural_cycle(integration_time_text, args.max_wait_seconds)
    ...
    if not observation['evidence']['observed']:
    ...
    if systemd_after['service'].get('FragmentSHA256') != service_fragment_sha_before:
        failures.append('service_unit_file_changed')
    if systemd_after['timer'].get('FragmentSHA256') != timer_fragment_sha_before:
    ...
    if db_after.get('query_only') is not True or db_after.get('total_changes') != 0:
    ...
    if not (match_count == signal_count == score_count):
        failures.append('derived_counts_not_equal')

    for lane_name, stats in [('market', market_after), ('adversarial', adversarial_after)]:
        ...
        if stats.get('duplicate_event_uid_count') != 0:
            failures.append(lane_name + '_jsonl_duplicate_event_uid')
        if stats.get('unsafe_events') != 0:
            failures.append(lane_name + '_jsonl_unsafe_events')

    required_json = [HOT_REFRESH_STATE, SUMMARY, DISPLAY, HOT, BRIDGE]
    missing_json = [str(path.relative_to(ROOT)) for path in required_json if not path.exists()]
    if missing_json:
        failures.append('required_runtime_json_missing:' + ','.join(missing_json))
    ...
    if not missing_json:
        ...
        if hot_refresh.get('decision') != 'OK_NEWS_HOT_INGRESS_BOUNDED_RUNTIME_REFRESH_V1':
            failures.append('hot_refresh_decision_not_ok')
        try:
            hot_generated = parse_dt(str(hot_refresh.get('generated_at_utc') or ''))
        ...
        for prefix, obj in [
            ...
            validate_authority(obj.get('authority'), prefix, failures)
        ...
        if not isinstance(queue, list) or not isinstance(queue_count, int):
            ...
            for item in queue:
                ...
                if item.get('gateway_decision') != 'REVIEW_ONLY':
                    failures.append('hot_queue_item_not_review_only')
                validate_authority(item.get('authority'), 'hot_item', failures)
    ...
    for name, source in ACTIVE_COPY_PAIRS.items():
        ...
        active_hashes[name] = {
            ...
            'match': bool(source_sha and source_sha == target_sha),
        }
        if not active_hashes[name]['match']:
            failures.append('active_panel_hash_mismatch:' + name)

    tracker_alignment: dict[str, Any] = {'exists': TRACKER.exists()}
    if tracker:
        tracker_alignment.update({
            'last_raw_count_seen': tracker.get('last_raw_count_seen'),
            'last_match_count_seen': tracker.get('last_match_count_seen'),
            'last_signal_count_seen': tracker.get('last_signal_count_seen'),
            'last_score_count_seen': tracker.get('last_score_count_seen'),
            'processed_news_uid_count': len(tracker.get('processed_news_uids') or []),
            'last_success_batch': tracker.get('last_success_batch'),
        })
        for tracker_key, table in [
            ('last_raw_count_seen', 'news_raw_feed_events'),
            ('last_match_count_seen', 'news_token_match_events'),
            ('last_signal_count_seen', 'news_signal_events'),
            ('last_score_count_seen', 'news_score_events_v1'),
        ]:
            value = tracker.get(tracker_key)
            current = counts_after.get(table)
            if isinstance(value, int) and isinstance(current, int) and value > current:
                failures.append('tracker_count_ahead_of_db:' + tracker_key)
    else:
        warnings.append('processed_tracker_missing_or_empty')

    if failures:
        raise RuntimeError('OBSERVATION_VALIDATION_FAILURE:' + '|'.join(failures))
    ...
    artifact: dict[str, Any] = {
        ...
        'result': {
            ...
            'observation_status': 'NATURAL_TIMER_FULL_CYCLE_OBSERVED_AND_VERIFIED',
            'news_operational_baseline_status': 'CLOSED_VERIFIED_BOUNDED_RUNTIME',
            ...
            'service_unit_sha_unchanged': systemd_after['service'].get('FragmentSHA256') == service_fragment_sha_before,
            'timer_unit_sha_unchanged': systemd_after['timer'].get('FragmentSHA256') == timer_fragment_sha_before,
            'db_before_observer': db_before,
            'db_after_observer': db_after,
            'db_delta_during_observer': db_delta_during_observer,
            'db_delta_from_integration_baseline': db_delta_from_integration,
            'derived_counts_equal': match_count == signal_count == score_count,
            ...
            'hot_refresh_state': {
                ...
                'generated_at_utc': hot_refresh.get('generated_at_utc'),
                'finished_at_utc': hot_refresh.get('finished_at_utc'),
                'summary': hot_refresh.get('summary'),
                'failures': hot_refresh.get('failures'),
            },
            'coverage_summary': {
            ...
            'dynamic_outputs_tracked': [],
            'dynamic_outputs_ignored': True,
            'local_runtime_files_preserved': True,
    ...
    active_work_unit = {
        'id': WORK_UNIT,
        'type': 'POST_ERA54_HOT_INGRESS_BOUND_RUNTIME_FIRST_OBSERVATION_AND_OPERATIONAL_BASELINE_CLOSURE',
        'artifact': CONTROL_REL,
        'module': TOOL_REL,
        'status': 'CLOSED',
        'next_step': NEXT_STEP,
    ...
    runtime['mode'] = 'NEWS_OPERATIONAL_BASELINE_CLOSED_VERIFIED_BOUNDED_RUNTIME'
    ...
    runtime['news_operational_state'] = {
        'status': 'OPERATIONAL_BASELINE_CLOSED_VERIFIED_BOUNDED_RUNTIME',
        'cold_producer': 'ACTIVE_BY_EXISTING_TIMER',
        'derived_pipeline': 'BOUND',
        'coverage_lanes': 'BOUND',
        'hot_ingress': 'BOUND_AND_NATURAL_CYCLE_VERIFIED',
        'active_panel_data_bridge': 'BOUND',
        'hot_queue_count': hot.get('hot_queue_count'),
        'market_indicator_count': summary.get('market_indicator_count'),
        'adversarial_count': summary.get('adversarial_count'),
        ...
        'future_evolution': 'BACKLOG_NOT_CURRENT_OPERATIONAL_BLOCKER',
    ...
    boot['new_chat_instruction'] = (
        'Read PROJECT_RUNTIME.json first. NEWS operational baseline is closed and verified after a natural timer cycle. '
        'Do not rebuild NEWS or reopen HBR. Proceed only to ' + NEXT_STEP + '. '
        'Hunter/trade/paper/live/execution authority remains false.'
    )
    ...
    boot['project']['mode'] = 'NEWS_OPERATIONAL_BASELINE_CLOSED_VERIFIED_BOUNDED_RUNTIME'
    ...
    roadmap['news_operational_baseline'] = {
        ...
        'era54': 'CLOSED_VERIFIED_NOAPI',
        'hbr_current_attempt': 'CLOSED_INCONCLUSIVE_ZERO_ELIGIBLE_INPUT',
        'natural_timer_cycle_observed': True,
        'hot_ingress_bound': True,
        'active_panel_bound': True,
        'future_news_evolution': 'BACKLOG',
        'next_safe_step': NEXT_STEP,
    }
...
HOT_QUEUE_COUNT={hot.get('hot_queue_count')}
HOT_QUEUE_BOUND=50
HUNTER_TRADE_PAPER_LIVE_EXECUTION_AUTHORITY=false
NEW_ERA_OPENED=false
NEXT_SAFE_STEP={NEXT_STEP}
<!-- POST_ERA54_HOT_FIRST_OBSERVATION_CURRENT_END -->'''

    update_markdown(ROOT.joinpath('03_ROADMAP.md').relative_to(ROOT).as_posix(), block)
    update_markdown(
        '04_ALMANAC.md',
        block,
...
- DB counts: `{json.dumps(counts_after, ensure_ascii=False, sort_keys=True)}`
...
- Authority: `hunter/trade/paper/live/execution=false`
...
NEWS is not being rebuilt. Its current operational baseline is closed and verified after the first natural timer-driven full cycle with bounded hot ingress and active panel data binding.
...
- Previous HEAD: `{current_head}`
- Natural timer cycle: `OBSERVED_VERIFIED`
- Service result: `{systemd_after['service'].get('Result')}`
- Service ExecMainStatus: `{systemd_after['service'].get('ExecMainStatus')}`
- Timer active/enabled: `{systemd_after['timer'].get('ActiveState')}/{systemd_after['timer'].get('EnabledState')}`
- DB integrity: `{db_after.get('integrity')}`
- DB counts: `{json.dumps(counts_after, ensure_ascii=False, sort_keys=True)}`
- Derived counts equal: `{str(match_count == signal_count == score_count).lower()}`
- Market indicator events: `{summary.get('market_indicator_count')}`
- Adversarial events: `{summary.get('adversarial_count')}`
- Hot queue: `{hot.get('hot_queue_count')}/50`
- Panel bridge: `{bridge.get('decision')}`
- NEWS operational baseline: `CLOSED_VERIFIED_BOUNDED_RUNTIME`
- Future NEWS evolution: `BACKLOG_NOT_CURRENT_OPERATIONAL_BLOCKER`
- Next safe step: `{NEXT_STEP}`

## Boundaries

The observer made no network/API call, no production DB write, no schema change, no service/timer configuration change, and granted no Hunter/trade/paper/live/execution authority. The observed existing timer cycle retained its pre-existing runtime role.
''',
    )
