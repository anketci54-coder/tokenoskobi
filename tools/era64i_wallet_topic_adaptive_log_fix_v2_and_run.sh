...
BACKUP="/root/era64i_runner_before_wallet_topic_fix_v2_$(date -u +%Y%m%dT%H%M%SZ).sh"
CHECK_FILE="/tmp/era64i_generated_tool_check_$$.py"
...
'''It samples ERC-20 Transfer logs for canonical BSC base and quote assets across a bounded 4,096-block historical range using wallet-topic filters'''),
]
...
for old,new in replacements:
    ...
    if count!=1:
        raise SystemExit(f'ERA64I_WALLET_TOPIC_FIX_V2_PATTERN_COUNT_INVALID:{count}:{old[:100]!r}')
    text=text.replace(old,new,1)

path.write_text(text,encoding='utf-8')
print('ERA64I_WALLET_TOPIC_ADAPTIVE_LOG_FIX_V2=APPLIED')
PY

python3 <<'PY_EXTRACT' > "$CHECK_FILE"
from pathlib import Path
...
PY_EXTRACT
python3 -m py_compile "$CHECK_FILE"
...
grep -q 'wallet_topics=\[wallet_topic(item) for item in query_wallets\]' "$TARGET"

git add "$TARGET"
git commit -m "ERA64: use bounded wallet-topic historical log filters"
git push origin main
[[ "$(git rev-parse HEAD)" == "$(git rev-parse origin/main)" ]]
[[ -z "$(git status --porcelain=v1)" ]]
rm -f "$BACKUP"

echo "ERA64I_WALLET_TOPIC_ADAPTIVE_LOG_FIX_V2=VERIFIED"
