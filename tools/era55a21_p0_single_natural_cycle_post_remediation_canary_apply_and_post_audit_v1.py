...
WORK_UNIT = "ERA55A_21_P0_SINGLE_NATURAL_CYCLE_POST_REMEDIATION_CANARY_APPLY_AND_POST_AUDIT"
RESULT = "OK_POST_REMEDIATION_SINGLE_NATURAL_CYCLE_PRODUCTION_CANARY_COMPLETED"
NEXT = "ERA55A_22_P0_POST_REMEDIATION_CANARY_RED_TEAM_GENERAL_PRODUCTION_ACTIVATION_DECISION"
SUBJECT = "ERA55A21_POST_REMEDIATION_CANARY | OK | GENERAL_ACTIVATION_BLOCKED"
...
ARTIFACT = ROOT / "data/control/era55a21_p0_single_natural_cycle_post_remediation_canary_apply_and_post_audit_v1.json"
REPORT = ROOT / "reports/LATEST_ERA55A21_P0_SINGLE_NATURAL_CYCLE_POST_REMEDIATION_CANARY_APPLY_AND_POST_AUDIT.md"
...
DROPIN = DROPIN_DIR / "90-era55a21-canary.conf"
RESULT_PATH = RUNTIME_ROOT / "era55a21_one_shot_result.json"
INVOCATION_GUARD = RUNTIME_ROOT / "era55a21_invocation.guard"
ORDER_LOG = RUNTIME_ROOT / "era55a21_order.log"
FULL_DISPLAY = RUNTIME_ROOT / "era55a21_full_candidate_display.json"
WRITER_LOCK = RUNTIME_ROOT / "era55a21_writer.lock"
...
def service_environment() -> dict[str, Any]:
    ...
    return {
        ...
        "canary_mode_enabled": "TOKENOSKOBI_A21_ONE_SHOT_HOT=1" in text,
        "result": next(
            (
                line.split("=", 1)[1]
...
def restore_mutable_outputs(
    metadata: dict[str, dict[str, Any]],
) -> None:
    original_paths = {Path(item["path"]) for item in metadata.values()}
    current_paths = set(mutable_output_paths())
    for path in sorted(current_paths - original_paths, key=lambda item: str(item), reverse=True):
        path.unlink()
    for item in metadata.values():
        path = Path(item["path"])
        backup = Path(item["backup"])
        path.parent.mkdir(parents=True, exist_ok=True)
        temp = path.parent / ("." + path.name + ".a21restore")
...
def restore_database_from_backup(backup: Path) -> None:
    for suffix in ("-wal", "-shm"):
        ...
        if candidate.exists():
            candidate.unlink()
    temp = DB.parent / ("." + DB.name + ".a21restore")
    shutil.copy2(backup, temp)
    os.replace(temp, DB)
...
def authorization() -> dict[str, Any]:
    a20 = load(A20)
    assert a20["status"] == (
        "CLOSED_ONE_POST_REMEDIATION_PRODUCTION_CANARY_AUTHORIZED"
    )
    assert a20["result"] == (
        "OK_ONE_POST_REMEDIATION_PRODUCTION_CANARY_AUTHORIZED"
    )
    auth = a20["authorization"]
    assert auth["one_post_remediation_production_canary_authorized"] is True
    assert auth["new_production_canary_authorized"] is True
    assert auth["second_production_canary_authorized"] is True
    ...
    assert contract["rollback_only_new_batch_on_failure"] is True
    assert contract["preserve_existing_batch_on_failure"] is True
    assert contract["runtime_only_systemd_dropin_required"] is True
    assert contract["timer_state_restore_required"] is True
    assert contract["general_activation_after_canary"] is False
    return a20


def one_shot_hot() -> int:
    token = os.environ.get("TOKENOSKOBI_A21_CANARY_TOKEN", "").strip()
    expected = os.environ.get("TOKENOSKOBI_A21_EXPECTED_HEAD", "").strip()
    result_path = Path(
        os.environ.get("TOKENOSKOBI_A21_RESULT_PATH", str(RESULT_PATH))
    )
    invocation_guard = Path(
        os.environ.get(
            "TOKENOSKOBI_A21_INVOCATION_GUARD",
            str(INVOCATION_GUARD),
        )
    )
    if not token or not expected:
        raise RuntimeError("A21_CANARY_ENVIRONMENT_INCOMPLETE")
    if git("rev-parse", "HEAD") != expected:
        raise RuntimeError("A21_CANARY_HEAD_MISMATCH")
    ...
    append_order("A21_ONE_SHOT_HOT_START")
    original = run(
    ...
    append_order(f"A21_ORIGINAL_HOT_END:{original.returncode}")
    if original.returncode != 0:
        raise RuntimeError(
            "A21_ORIGINAL_HOT_FAILED:"
    ...
    if not isinstance(legacy_queue, list):
        raise RuntimeError("A21_LEGACY_HOT_QUEUE_NOT_LIST")

    tools_path = str(ROOT / "tools")
    if tools_path not in sys.path:
        sys.path.insert(0, tools_path)
    extractor = load_module("a21_extractor", EXTRACTOR)
    adapter = load_module("a21_adapter", ADAPTER)
    if adapter.POLICY_VERSION != LEDGER_POLICY:
        raise RuntimeError("A21_LEDGER_POLICY_MISMATCH")
    ...
    if not (0 < len(legacy_queue) <= 50):
        raise RuntimeError("A21_LEGACY_QUEUE_BOUND_FAILED")
    if not (len(legacy_queue) <= source_count <= MAX_SOURCE_ROWS):
        raise RuntimeError("A21_SOURCE_BOUND_FAILED")
    if accounted != source_count:
        raise RuntimeError("A21_SOURCE_ACCOUNTING_FAILED")
    ...
    if set(before_map) != {expected_baseline_uid}:
        raise RuntimeError("A21_BASELINE_BATCH_SET_DRIFT")
    ...
    append_order("A21_LEDGER_WRITE_DONE:" + writer_status)
    if writer_status != "COMMITTED":
        raise RuntimeError("A21_LEDGER_WRITE_NOT_COMMITTED:" + writer_status)

    final_hot = load(HOT)
    final_queue = final_hot.get("hot_queue")
    if canonical(final_queue) != canonical(legacy_queue):
        raise RuntimeError("A21_FINAL_QUEUE_PARITY_FAILED")
    ...
    append_order(f"A21_PANEL_BRIDGE_END:{bridge.returncode}")
    if bridge.returncode != 0:
        raise RuntimeError(
            "A21_PANEL_BRIDGE_FAILED:"
    ...
    if bridge_state.get("decision") != "OK_NEWS_ACTIVE_PANEL_DATA_BRIDGE_APPLIED":
        raise RuntimeError("A21_PANEL_BRIDGE_DECISION_NOT_OK")
    ...
    if not isinstance(hash_match, dict) or not hash_match or not all(
        ...
        raise RuntimeError("A21_PANEL_BRIDGE_HASH_MISMATCH")
    if sha(HOT) != sha(PANEL_HOT):
        raise RuntimeError("A21_PANEL_HOT_HASH_MISMATCH")

    after = database_inventory(DB)
    after_map = batch_map(after)
    if set(after_map) != {expected_baseline_uid, expected_new_uid}:
        raise RuntimeError("A21_POSTWRITE_BATCH_SET_INVALID")
    new_batch = after_map[expected_new_uid]
    if new_batch["source_candidate_count"] != source_count:
        raise RuntimeError("A21_NEW_BATCH_SOURCE_COUNT_MISMATCH")
    if new_batch["ledger_rows"] != source_count:
        raise RuntimeError("A21_NEW_BATCH_LEDGER_COUNT_MISMATCH")

    payload = {
        "schema_version": "1.0",
        "token": token,
        "status": "OK_A21_ONE_SHOT_HOT_WRAPPER_COMPLETED",
    ...
    append_order("A21_ONE_SHOT_HOT_END:0")
...
def orchestrate() -> int:
    ...
    if ARTIFACT.exists():
        raise RuntimeError("A21_ARTIFACT_ALREADY_EXISTS")
    if DROPIN.exists():
        raise RuntimeError("A21_DROPIN_ALREADY_PRESENT")

    a20 = authorization()
    contract = a20["canary_execution_contract"]
    baseline_uid = str(contract["existing_batch_uid_must_be_preserved"])
    expected_new_uid = str(contract["prospective_batch_uid"])
    expected_new_rows = int(contract["expected_new_ledger_rows"])
    expected_total_rows = int(contract["expected_total_ledger_rows_on_success"])
    expected_total_batches = int(contract["expected_total_batch_rows_on_success"])
    ...
    if timer_before["active"] != "active" or timer_before["enabled"] != "enabled":
        raise RuntimeError("A21_TIMER_PRECONDITION_FAILED")
    if environment_initial["writer_enabled"]:
        raise RuntimeError("A21_WRITER_ALREADY_ENABLED")
    if environment_initial["runner_lock_enabled"]:
        raise RuntimeError("A21_RUNNER_LOCK_ALREADY_ENABLED")
    if environment_initial["any_hot_override"]:
        raise RuntimeError("A21_HOT_OVERRIDE_ALREADY_ENABLED")
    if environment_initial["canary_mode_enabled"]:
        raise RuntimeError("A21_CANARY_MODE_ALREADY_ENABLED")
    ...
    rollback_guard = load_module("a21_rollback_guard", ROLLBACK_GUARD)
    if rollback_guard.POLICY_VERSION != ROLLBACK_POLICY:
        raise RuntimeError("A21_ROLLBACK_POLICY_MISMATCH")
    ...
    def rollback_and_restore(original_error: str) -> dict[str, Any]:
        ...
        if len(new_uids) > 1:
            ...
            if new_uid == baseline_uid:
                ...
                result = rollback_guard.rollback_committed_batch(
                    ...
                    new_uid,
                    original_error=original_error,
                    archive_location="rollback://era55a21/postcommit-failure",
                )
                try:
        ...
        if full_restore_required:
            if db_backup is None:
                raise RuntimeError("A21_DATABASE_BACKUP_MISSING")
        ...
        if after_restore != before_inventory:
            ...
            result["emergency_full_database_restore"] = True
        if after_restore != before_inventory:
            raise RuntimeError("A21_DATABASE_RESTORE_PARITY_FAILED")
    ...
    def emergency_cleanup() -> None:
        ...
        if lifecycle["canary_started"] and not lifecycle["finalized"] and not lifecycle["rollback_done"]:
            try:
                ...
                rollback_and_restore("A21_EMERGENCY_CLEANUP")
            except Exception:
                pass
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            raise RuntimeError("A21_SERVICE_DID_NOT_BECOME_INACTIVE")
        ...
        if environment_before["writer_enabled"] or environment_before["runner_lock_enabled"]:
            raise RuntimeError("A21_RUNTIME_FLAGS_ACTIVE_AFTER_TIMER_PAUSE")
        if environment_before["any_hot_override"] or environment_before["canary_mode_enabled"]:
            raise RuntimeError("A21_RUNTIME_OVERRIDE_ACTIVE_AFTER_TIMER_PAUSE")
        ...
        if before_inventory["batch_rows"] != 1 or before_inventory["ledger_rows"] != 106:
            raise RuntimeError("A21_BASELINE_DATABASE_COUNTS_DRIFT")
        if set(before_map) != {baseline_uid}:
            raise RuntimeError("A21_BASELINE_BATCH_UID_DRIFT")
        baseline_before = before_map[baseline_uid]
        if baseline_before["status"] != "COMMITTED":
            raise RuntimeError("A21_BASELINE_NOT_COMMITTED")
        if baseline_before["policy_version"] != LEDGER_POLICY:
            raise RuntimeError("A21_BASELINE_POLICY_MISMATCH")
        if baseline_before["ledger_rows"] != 106:
            raise RuntimeError("A21_BASELINE_LEDGER_COUNT_DRIFT")
        if before_inventory["integrity_check"] != "ok":
            raise RuntimeError("A21_BASELINE_INTEGRITY_FAILED")
        if before_inventory["quick_check"] != "ok":
            raise RuntimeError("A21_BASELINE_QUICK_CHECK_FAILED")
        if before_inventory["foreign_key_check_rows"] != 0:
            raise RuntimeError("A21_BASELINE_FOREIGN_KEY_FAILED")
        ...
        DROPIN.write_text(
            "\n".join(
                [
                    ...
                    'Environment="TOKENOSKOBI_A21_ONE_SHOT_HOT=1"',
                    f'Environment="TOKENOSKOBI_A21_CANARY_TOKEN={token}"',
                    f'Environment="TOKENOSKOBI_A21_EXPECTED_HEAD={EXPECTED_HEAD}"',
                    f'Environment="TOKENOSKOBI_A21_RESULT_PATH={RESULT_PATH}"',
                    f'Environment="TOKENOSKOBI_A21_INVOCATION_GUARD={INVOCATION_GUARD}"',
        ...
        if not (
            ...
            raise RuntimeError("A21_RUNTIME_DROPIN_NOT_ACTIVE")
        ...
        while time.time() < deadline:
            ...
            raise RuntimeError("A21_SERVICE_COMPLETION_TIMEOUT")

        after_service_environment = service_environment()
        if after_service_environment["result"] not in {"success", ""}:
            raise RuntimeError("A21_SERVICE_RESULT_NOT_SUCCESS")
        if after_service_environment["exec_main_status"] not in {"0", ""}:
            raise RuntimeError("A21_SERVICE_MAIN_STATUS_NOT_ZERO")
        if not RESULT_PATH.exists():
            raise RuntimeError("A21_ONE_SHOT_RESULT_MISSING")
        one_shot = load(RESULT_PATH)
        if one_shot.get("token") != token:
            raise RuntimeError("A21_ONE_SHOT_TOKEN_MISMATCH")
        ...
        required_markers = [
            ...
            "A21_LEDGER_WRITE_DONE:COMMITTED",
            "A21_PANEL_BRIDGE_END:0",
            "A21_ONE_SHOT_HOT_END:0",
            "HOT_END:0",
        ]
        positions: list[int] = []
        for marker in required_markers:
            if marker not in order:
                raise RuntimeError(f"A21_ORDER_MARKER_MISSING:{marker}")
            positions.append(order.index(marker))
        if positions != sorted(positions):
            raise RuntimeError("A21_ORDER_SEQUENCE_INVALID")
        if order.count("A21_ONE_SHOT_HOT_START") != 1:
            raise RuntimeError("A21_ONE_SHOT_INVOCATION_COUNT_INVALID")
        recovery_markers = [
            marker for marker in order if marker.startswith("RECOVERY_DONE:")
        ]
        if len(recovery_markers) != 1:
            raise RuntimeError("A21_RECOVERY_MARKER_COUNT_INVALID")
        if recovery_markers[0] not in {
            "RECOVERY_DONE:OUTPUT_ALREADY_MATCHED",
            "RECOVERY_DONE:RECOVERED",
        }:
            raise RuntimeError("A21_RECOVERY_MARKER_INVALID:" + recovery_markers[0])

        after_inventory = database_inventory(DB)
        after_map = batch_map(after_inventory)
        if after_inventory["batch_rows"] != expected_total_batches:
            raise RuntimeError("A21_TOTAL_BATCH_COUNT_MISMATCH")
        if after_inventory["ledger_rows"] != expected_total_rows:
            raise RuntimeError("A21_TOTAL_LEDGER_COUNT_MISMATCH")
        if set(after_map) != {baseline_uid, expected_new_uid}:
            raise RuntimeError("A21_FINAL_BATCH_SET_INVALID")
        if after_map[baseline_uid] != baseline_before:
            raise RuntimeError("A21_BASELINE_BATCH_MUTATED")
        new_batch = after_map[expected_new_uid]
        if new_batch["status"] != "COMMITTED":
            raise RuntimeError("A21_NEW_BATCH_NOT_COMMITTED")
        if new_batch["policy_version"] != LEDGER_POLICY:
            raise RuntimeError("A21_NEW_BATCH_POLICY_MISMATCH")
        if new_batch["source_candidate_count"] != expected_new_rows:
            raise RuntimeError("A21_NEW_BATCH_SOURCE_COUNT_MISMATCH")
        if new_batch["ledger_rows"] != expected_new_rows:
            raise RuntimeError("A21_NEW_BATCH_LEDGER_COUNT_MISMATCH")
        if sum(new_batch["disposition_counts"].values()) != expected_new_rows:
            raise RuntimeError("A21_NEW_BATCH_DISPOSITION_ACCOUNTING_FAILED")
        if after_inventory["integrity_check"] != "ok":
            raise RuntimeError("A21_POST_INTEGRITY_FAILED")
        if after_inventory["quick_check"] != "ok":
            raise RuntimeError("A21_POST_QUICK_CHECK_FAILED")
        if after_inventory["foreign_key_check_rows"] != 0:
            raise RuntimeError("A21_POST_FOREIGN_KEY_FAILED")

        if one_shot["baseline_batch_uid"] != baseline_uid:
            raise RuntimeError("A21_RESULT_BASELINE_UID_MISMATCH")
        if one_shot["new_batch_uid"] != expected_new_uid:
            raise RuntimeError("A21_RESULT_NEW_UID_MISMATCH")
        if one_shot["source_candidate_count"] != expected_new_rows:
            raise RuntimeError("A21_RESULT_SOURCE_COUNT_MISMATCH")
        if one_shot["source_accounted"] != expected_new_rows:
            raise RuntimeError("A21_RESULT_SOURCE_ACCOUNTING_MISMATCH")
        if sha(HOT) != one_shot["hot_output_sha256"]:
            raise RuntimeError("A21_FINAL_HOT_HASH_MISMATCH")
        if sha(PANEL_HOT) != sha(HOT):
            raise RuntimeError("A21_FINAL_PANEL_HOT_HASH_MISMATCH")
        bridge_state = load(BRIDGE_STATE)
        if bridge_state.get("decision") != "OK_NEWS_ACTIVE_PANEL_DATA_BRIDGE_APPLIED":
            raise RuntimeError("A21_FINAL_BRIDGE_DECISION_FAILED")
        if not bridge_state.get("hash_match") or not all(
            value is True for value in bridge_state["hash_match"].values()
        ):
            raise RuntimeError("A21_FINAL_BRIDGE_HASH_MATCH_FAILED")

        remove_dropin()
        environment_after_cleanup = service_environment()
        if environment_after_cleanup["writer_enabled"]:
            raise RuntimeError("A21_WRITER_FLAG_STILL_ENABLED")
        if environment_after_cleanup["runner_lock_enabled"]:
            raise RuntimeError("A21_LOCK_FLAG_STILL_ENABLED")
        if environment_after_cleanup["any_hot_override"]:
            raise RuntimeError("A21_HOT_OVERRIDE_STILL_ENABLED")
        if environment_after_cleanup["canary_mode_enabled"]:
            raise RuntimeError("A21_CANARY_MODE_STILL_ENABLED")
        if DROPIN.exists():
            raise RuntimeError("A21_DROPIN_STILL_PRESENT")
        timer_post_audit_paused = systemctl_state(TIMER)
        if timer_post_audit_paused["active"] != "inactive":
            raise RuntimeError("A21_TIMER_NOT_PAUSED_DURING_POST_AUDIT")
        service_after = systemctl_state(SERVICE)
        if service_after["active"] not in {"inactive", "failed"}:
            raise RuntimeError("A21_SERVICE_NOT_INACTIVE_AFTER_CYCLE")
        ...
        if timer_after["active"] != timer_before["active"]:
            raise RuntimeError("A21_TIMER_ACTIVE_STATE_NOT_RESTORED")
        if timer_after["enabled"] != timer_before["enabled"]:
            raise RuntimeError("A21_TIMER_ENABLED_STATE_CHANGED")
        ...
        artifact = {
            ...
            "status": "CLOSED_POST_REMEDIATION_SINGLE_NATURAL_CYCLE_CANARY_OK",
            "result": RESULT,
            ...
            "service_environment_after_cleanup": {
                key: value
                for key, value in environment_after_cleanup.items()
                if key != "raw"
            },
            "runtime_cleanup": {
                ...
                "canary_mode_disabled": not environment_after_cleanup["canary_mode_enabled"],
            ...
            "database_post_audit": {
                "expected_total_batch_rows": expected_total_batches,
                "actual_total_batch_rows": after_inventory["batch_rows"],
                "expected_total_ledger_rows": expected_total_rows,
                "actual_total_ledger_rows": after_inventory["ledger_rows"],
                "new_batch_source_rows": new_batch["source_candidate_count"],
                "new_batch_ledger_rows": new_batch["ledger_rows"],
                "all_new_source_rows_accounted": (
                    new_batch["ledger_rows"] == new_batch["source_candidate_count"]
                ),
            ...
            "authorization": {
                "one_post_remediation_production_canary_authorized": False,
                "one_post_remediation_production_canary_consumed": True,
        ...
        REPORT.write_text(
            f"""# ERA55A21 Post-Remediation Single Natural Cycle Production Canary

- Status: `CLOSED_POST_REMEDIATION_SINGLE_NATURAL_CYCLE_CANARY_OK`
- Result: `{RESULT}`
- Baseline batch UID: `{baseline_uid}`
- Baseline batch preserved: `true`
- New batch UID: `{expected_new_uid}`
- New source candidates: `{expected_new_rows}`
- New source accounted: `{new_batch['ledger_rows']}`
- Total batch rows: `{after_inventory['batch_rows']}`
- Total ledger rows: `{after_inventory['ledger_rows']}`
- Unobservable rows: `0`
...
""",
        ...
        current.update(
            {
                "mode": "ERA55A21_POST_REMEDIATION_SINGLE_CYCLE_CANARY_COMPLETED",
                ...
                "active_work_unit": {
                    "id": WORK_UNIT,
                    "type": "ERA55_P0_POST_REMEDIATION_SINGLE_CYCLE_CANARY_APPLY_POST_AUDIT",
                ...
                "next_safe_step": {
                    ...
                    "parent": "ERA55_RUNTIME_OPTIMIZATION",
                    "purpose": (
                        "Review the successful remediated production canary and decide "
                        "general production writer activation separately."
                    ),
        ...
        event_id = "ERA55A21_POST_REMEDIATION_SINGLE_CYCLE_CANARY_V1"
        if not any(
            ...
            events.append(
                {
                    ...
                    "era": "ERA55",
                    "work_unit": WORK_UNIT,
                    "event": "POST_REMEDIATION_SINGLE_NATURAL_CYCLE_CANARY_APPLY_POST_AUDIT",
                    "status": artifact["status"],
                    "result": RESULT,
                    "artifact": str(ARTIFACT.relative_to(ROOT)),
                    "baseline_batch_uid": baseline_uid,
                    "baseline_batch_preserved": True,
                    "new_batch_uid": expected_new_uid,
                    "new_batch_source_rows": expected_new_rows,
                    "new_batch_ledger_rows": new_batch["ledger_rows"],
                    "total_batch_rows": after_inventory["batch_rows"],
                    "total_ledger_rows": after_inventory["ledger_rows"],
                    "unobservable_rows": 0,
...
BASELINE_BATCH_PRESERVED=true
NEW_BATCH_UID={expected_new_uid}
NEW_SOURCE_CANDIDATES={expected_new_rows}
NEW_SOURCE_ACCOUNTED={new_batch['ledger_rows']}
...
POST_REMEDIATION_CANARY_CONSUMED=true
GENERAL_PRODUCTION_WRITER_ACTIVATION_AUTHORIZED=false
...
The guarded post-remediation canary completed once. General activation remains blocked.""",
        )
        master = replace_section(
            master,
...
- Do not start Option B or close P0 F1.""",
        ...
        handoff = replace_section(
            ...
            f"""- Post-remediation production canary: `COMPLETED_AND_CONSUMED`.
...
2. Independently audit A21 batch preservation, new-batch accounting and runtime cleanup.
...
5. Keep Option B blocked until the production decision is sealed.""",
        ...
        marker = "## ERA55A_21 POST-REMEDIATION SINGLE-CYCLE PRODUCTION CANARY"
...
- Baseline batch preserved: `true`
- New batch UID: `{expected_new_uid}`
- New source rows: `{expected_new_rows}`
- Total batch rows: `{after_inventory['batch_rows']}`
- Total ledger rows: `{after_inventory['ledger_rows']}`
...
"""
        ...
        if not git("diff", "--cached", "--name-only"):
            raise RuntimeError("A21_NO_STAGED_CHANGES")
        git("commit", "-m", SUBJECT)
        lifecycle["finalized"] = True
        atexit.unregister(emergency_cleanup)

        print("ERA55A21_POST_REMEDIATION_CANARY=SUCCESS")
        print("RESULT=" + RESULT)
        print("BASELINE_BATCH_UID=" + baseline_uid)
        print("BASELINE_BATCH_PRESERVED=true")
        print("NEW_BATCH_UID=" + expected_new_uid)
        print("NEW_SOURCE_CANDIDATES=" + str(expected_new_rows))
        print("NEW_SOURCE_ACCOUNTED=" + str(new_batch["ledger_rows"]))
        ...
        print("POST_REMEDIATION_CANARY_CONSUMED=true")
        ...
        lifecycle["rollback_done"] = True
        atexit.unregister(emergency_cleanup)
        print("A21_FAILURE=" + original_error, file=sys.stderr)
        print(
            "A21_ROLLBACK="
...
def main() -> int:
    if os.environ.get("TOKENOSKOBI_A21_ONE_SHOT_HOT", "0").strip() == "1":
        return one_shot_hot()
    return orchestrate()
