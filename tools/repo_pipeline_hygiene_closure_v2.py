...
ONE_OFF_FILES = (
    ...
    'tools/repo_pipeline_hygiene_closure_v1.py',
...
def systemctl_props(unit: str) -> dict[str, str]:
    props = ('LoadState', 'ActiveState', 'SubState', 'UnitFileState', 'Result', 'FragmentPath')
...
def finish_pending_push_if_needed() -> bool:
    pointer = load(ROOT / 'PROJECT_RUNTIME.json').get('canonical_runtime_pointer', {})
...
def validate_phase9_proof() -> dict[str, Any]:
    ...
    for key, value in expected.items():
        if checks.get(key) != value:
            raise RuntimeError(f'PHASE9_PROOF_MISMATCH:{key}:{checks.get(key)}')
    return data


def protected_inventory() -> tuple[list[str], list[str], list[str], dict[str, str]]:
    ...
    disposable = list(classification['DISPOSABLE'])
    if len(active_runtime) != 14 or len(active_data) != 12:
        raise RuntimeError(f'PROTECTED_COUNT_MISMATCH:{len(active_runtime)}:{len(active_data)}')
    if len(disposable) != 25:
        raise RuntimeError(f'DISPOSABLE_MANIFEST_COUNT_MISMATCH:{len(disposable)}')
    hashes: dict[str, str] = {}
    ...
    return active_runtime, active_data, disposable, hashes


def remove_manifest_disposable(paths: list[str]) -> tuple[list[str], list[str]]:
    removed: list[str] = []
    ...
    for rel in paths:
        ...
        try:
            ...
            raise RuntimeError('DISPOSABLE_OUTSIDE_ROOT:' + rel) from exc
        if '__pycache__' not in path.parts and path.suffix.lower() not in {'.pyc', '.pyo', '.tmp', '.swp', '.bak', '.orig'}:
            raise RuntimeError('DISPOSABLE_MANIFEST_UNSAFE_PATH:' + rel)
        if path.is_file():
            path.unlink()
            removed.append(rel)
        else:
...
def archive_one_off_files() -> tuple[list[dict[str, str]], list[str], list[Path]]:
...
def post_classification(expected_runtime: list[str], expected_data: list[str]) -> dict[str, Any]:
    ...
    for base_name in ('tools', 'tests', 'runtime', 'data', 'reports'):
        ...
        for full in base.rglob('*'):
            ...
            if rel in file_set:
                continue
            if '__pycache__' in full.parts or full.suffix.lower() in {'.pyc', '.pyo', '.tmp', '.swp', '.bak', '.orig'}:
...
def run_post_stress() -> dict[str, Any]:
    ...
    if data.get('source_hash_verified') is not True:
        raise RuntimeError('POST_HYGIENE_STRESS_HASH_NOT_VERIFIED')
    return {
        'verdict': 'OK',
        'scenario_count': 10,
...
def main() -> int:
    if finish_pending_push_if_needed():
        return 0

    base_head = require_clean_synced()
    pointer = load(ROOT / 'PROJECT_RUNTIME.json')['canonical_runtime_pointer']
    ...
    dependency = validate_phase9_proof()

    say('STEP=FREEZE_ACTIVE_RUNTIME_HASHES')
    active_runtime, active_data, disposable_manifest, protected_before = protected_inventory()
    db_before = sha256(PRODUCTION_DB)
    phase9_timer_before = systemctl_props(PHASE9_TIMER)
    phase9_service_before = systemctl_props(PHASE9_SERVICE)
    news_timer_before = systemctl_props(NEWS_TIMER)
    transient_before = systemctl_props(TRANSIENT_UNIT)
    ...
    if phase9_timer_before.get('ActiveState') != 'active' or phase9_timer_before.get('UnitFileState') != 'enabled':
        raise RuntimeError('PHASE9_TIMER_NOT_ACTIVE_ENABLED')
    if news_timer_before.get('ActiveState') != 'active' or news_timer_before.get('UnitFileState') != 'enabled':
        raise RuntimeError('NEWS_TIMER_NOT_ACTIVE_ENABLED')
    ...
    try:
        ...
        run(['systemctl', 'disable', '--now', PHASE9_TIMER], timeout=60)
        timer_mutated = True
        phase9_timer_after = systemctl_props(PHASE9_TIMER)
        if phase9_timer_after.get('ActiveState') != 'inactive' or phase9_timer_after.get('UnitFileState') != 'disabled':
            raise RuntimeError('PHASE9_TIMER_DISABLE_VERIFY_FAILED')
        ...
        transient_after = systemctl_props(TRANSIENT_UNIT)
        if transient_after.get('ActiveState') == 'failed':
            raise RuntimeError('TRANSIENT_UNIT_STILL_FAILED')

        say('STEP=REMOVE_25_MANIFEST_DISPOSABLE_FILES')
        removed, already_absent = remove_manifest_disposable(disposable_manifest)

        say('STEP=ARCHIVE_ONE_OFF_TOOLS')
        moved, absent, destinations = archive_one_off_files()

        say('STEP=RECLASSIFY_ACTIVE_SURFACE')
        classification = post_classification(active_runtime, active_data)

        say('STEP=RUN_POST_HYGIENE_STRESS')
        stress = run_post_stress()

        say('STEP=VERIFY_PROTECTED_HASHES')
        protected_after = {rel: sha256(ROOT / rel) for rel in active_runtime + active_data}
        if protected_after != protected_before:
            raise RuntimeError('PROTECTED_ACTIVE_SURFACE_HASH_CHANGED')
        db_after = sha256(PRODUCTION_DB)
        if db_after != db_before:
            raise RuntimeError('PRODUCTION_DB_HASH_CHANGED')

        phase9_service_after = systemctl_props(PHASE9_SERVICE)
        news_timer_after = systemctl_props(NEWS_TIMER)
        ...
        artifact = {
            ...
            'disposable_cleanup': {
                'manifest_count': len(disposable_manifest),
            ...
            'one_off_archive': {
                ...
                'reusable_tools_preserved': [
                    ...
                    'tests/general_runtime_stress_harness_v1.py',
                    'tools/repo_pipeline_hygiene_closure_v2.py',
                ],
            },
            'active_surface_post_hygiene': classification,
            'post_hygiene_stress': stress,
            'protected_active_runtime_count': 14,
            'protected_active_runtime_data_count': 12,
            'protected_hashes_before': protected_before,
            'protected_hashes_after': protected_after,
            'protected_hashes_unchanged': True,
            'production_db_hash_before': db_before,
            'production_db_hash_after': db_after,
            'production_database_mutation': False,
        ...
        protected_final = {rel: sha256(ROOT / rel) for rel in active_runtime + active_data}
        if protected_final != protected_before:
            raise RuntimeError('CANONICAL_UPDATE_TOUCHED_PROTECTED_RUNTIME_DATA')

        say('STEP=POST_AUDIT')
        run(['git', 'diff', '--check'])
        pointer_after = load(ROOT / 'PROJECT_RUNTIME.json')['canonical_runtime_pointer']
        if pointer_after.get('next_safe_step') != NEXT or pointer_after.get('era57_opened') is not False:
            raise RuntimeError('CANONICAL_POST_AUDIT_FAILED')
        if pointer_after.get('repo_hygiene_ready_for_era57') is not True or pointer_after.get('pipeline_hygiene_ready_for_era57') is not True:
            raise RuntimeError('HYGIENE_READINESS_NOT_CLOSED')

        say('STEP=GIT_COMMIT_AND_SEAL')
        run(['git', 'add', 'PROJECT_RUNTIME.json', 'PROJECT_HISTORY.json', '04_ALMANAC.md', '06_PROJECT_MASTER_STATE.md', '07_PROJECT_HANDOFF.md', ARTIFACT_REL])
        ...
        say(f'DISPOSABLE_MANIFEST={len(disposable_manifest)}')
        say(f'DISPOSABLE_REMOVED={len(removed)}')
        say(f'DISPOSABLE_ALREADY_ABSENT={len(already_absent)}')
        ...
        if not committed:
            rollback(base_head, destinations, timer_mutated)
        else:
