#!/usr/bin/env python3
from __future__ import annotations

import json, os, subprocess, tempfile
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

ROOT=Path('/root/tokenoskobi_clean_v1')
WORK='ERA56_GLOBAL_INTELLIGENCE_CACHE_OPENING_DECISION'
RESULT='OK_ERA56_OPENED_OWNERSHIP_AND_OVERLAP_CONTRACT_REQUIRED'
NEXT='ERA56A_GLOBAL_CACHE_OWNERSHIP_OVERLAP_AND_REBUILD_CONTRACT'
SUBJECT='ERA56_OPENING | OK | GLOBAL_INTELLIGENCE_CACHE'
SEAL_TAG='ERA55_FINAL_SEAL'
SEAL_COMMIT='f22ce4f07788ec7fbe22a72f872467705b72db5a'

RUNTIME=ROOT/'PROJECT_RUNTIME.json'; ROADMAP=ROOT/'data/tokenoskobi_v1_v8_master_era_roadmap.json'
MASTER=ROOT/'06_PROJECT_MASTER_STATE.md'; HANDOFF=ROOT/'07_PROJECT_HANDOFF.md'; ALMANAC=ROOT/'04_ALMANAC.md'
HISTORY=ROOT/'PROJECT_HISTORY.json'; TK_AI=ROOT/'reports/LATEST_TK_AI_HANDOFF.md'; MACHINE=ROOT/'data/control/latest_tk_machine_state.json'
ARTIFACT=ROOT/'data/control/era56_global_intelligence_cache_opening_v1.json'


def run(args:list[str],check:bool=True): return subprocess.run(args,cwd=ROOT,text=True,capture_output=True,check=check,timeout=90)
def git(*args:str)->str: return run(['git',*args]).stdout.strip()
def load(p:Path)->dict[str,Any]:
    v=json.loads(p.read_text(encoding='utf-8'))
    if not isinstance(v,dict): raise RuntimeError('JSON_OBJECT_REQUIRED:'+str(p))
    return v

def dump(p:Path,v:dict[str,Any]):
    p.parent.mkdir(parents=True,exist_ok=True); fd,tmp=tempfile.mkstemp(prefix=p.name+'.',dir=p.parent)
    try:
        with os.fdopen(fd,'w',encoding='utf-8') as h:
            json.dump(v,h,ensure_ascii=False,indent=2,sort_keys=True); h.write('\n'); h.flush(); os.fsync(h.fileno())
        os.replace(tmp,p)
    finally:
        if os.path.exists(tmp): os.unlink(tmp)

def sec(text:str,heading:str,body:str)->str:
    s=text.find(heading)
    if s<0: raise RuntimeError('HEADING_MISSING:'+heading)
    e=text.find('\n## ',s+len(heading)); e=len(text) if e<0 else e
    return text[:s]+heading+'\n\n'+body.rstrip()+'\n'+text[e:]

def find_era(roadmap:dict[str,Any],era_id:str)->dict[str,Any]:
    for v in roadmap.get('versions',[]):
        if v.get('id')=='V3':
            for e in v.get('children',[]):
                if e.get('id')==era_id: return e
    raise RuntimeError('ERA_NOT_FOUND:'+era_id)

def main()->int:
    if git('status','--short'): raise RuntimeError('WORKTREE_NOT_CLEAN')
    expected=os.environ.get('TOKENOSKOBI_EXPECTED_HEAD','').strip()
    if expected and git('rev-parse','HEAD')!=expected: raise RuntimeError('HEAD_MISMATCH')
    if git('rev-list','-n1',SEAL_TAG)!=SEAL_COMMIT: raise RuntimeError('ERA55_SEAL_MISMATCH')

    runtime=load(RUNTIME); roadmap=load(ROADMAP); p=runtime['canonical_runtime_pointer']; e55=find_era(roadmap,'ERA55'); e56=find_era(roadmap,'ERA56')
    checks={
      'era55_closed':p.get('era55_closed') is True and e55.get('status')=='CLOSED',
      'era56_not_open':p.get('era56_opened') is False and e56.get('status')=='PLANNED',
      'next_step_matches':p.get('next_safe_step')==WORK,
      'cleanup_closed':p.get('last_completed')=='ERA55_POST_CLOSE_CLEANUP_AND_ERA56_ENTRY_HARDENING',
      'runner_lock_enabled':p.get('runner_lock_enabled') is True,
      'writer_active':p.get('production_ledger_writer_active') is True,
      'p0_closed':p.get('p0_f1_closed') is True,
      'option_b_blocked':p.get('option_b_authorized') is False and p.get('wal_apply_authorized') is False,
    }
    if not all(checks.values()): raise RuntimeError('OPENING_CHECK_FAILED:'+','.join(k for k,v in checks.items() if not v))

    ts=datetime.now(timezone.utc).isoformat(); rel=str(ARTIFACT.relative_to(ROOT))
    ownership={
      'source_of_truth':'Existing canonical ledgers, runtime DB tables and producer artifacts remain authoritative.',
      'era56_role':'Derived immutable global snapshot/cache/readmodel only.',
      'writes_to_source_authority':False,
      'duplicates_ledger':False,
      'duplicates_panel_bridge':False,
      'cache_is_rebuildable':True,
      'cache_is_disposable_without_source_loss':True,
      'stale_detection_required':True,
      'content_hash_required':True,
      'atomic_publish_required':True,
      'single_writer_required':True,
      'production_apply_authorized':False,
    }
    dump(ARTIFACT,{'schema':'era56_global_intelligence_cache_opening_v1','timestamp_utc':ts,'work_unit':WORK,'status':'OPENED_BOUNDED_DESIGN_ONLY','result':RESULT,'checks':checks,'ownership_contract':ownership,'production_mutation':False,'db_mutation':False,'service_timer_mutation':False,'era56_opened':True,'next_safe_step':NEXT})

    runtime['current_era']='ERA56'; runtime['current_era_status']='OPEN'; runtime['current_problem']={'code':'ERA56_GLOBAL_CACHE_OWNERSHIP_OVERLAP_REBUILD_CONTRACT_PENDING','severity':'P1','evidence':rel}
    p.update({'project_status':'ACTIVE_ERA56_GLOBAL_INTELLIGENCE_CACHE','current_era':'ERA56_GLOBAL_INTELLIGENCE_CACHE','current_stage':'ERA56_OPENED_BOUNDED_DESIGN_ONLY','last_completed':WORK,'last_result':RESULT,'last_artifact':rel,'era55_closed':True,'era56_opened':True,'era56_production_apply_authorized':False,'next_safe_step':NEXT,'updated_at_utc':ts})
    runtime['current_state']={'project_status':'ACTIVE','runtime_status':'ERA56_OPEN_DESIGN_ONLY','mode':'ERA56_GLOBAL_INTELLIGENCE_CACHE','last_action':{'task':WORK,'result':RESULT,'artifact':rel,'timestamp':ts},'current_problem':runtime['current_problem'],'next_safe_step':{'id':NEXT,'status':'READY','human_authorization_required':True,'production_mutation_authorized':False},'updated_at':ts}
    runtime['current_work_unit']={'id':WORK,'status':'CLOSED_VERIFIED','result':RESULT,'artifact':rel,'production_mutation':False,'next_step':NEXT}
    dump(RUNTIME,runtime)

    e56.update({'status':'OPEN','opened_at_utc':ts,'opening_artifact':rel,'active_stage':'ERA56_OPENED_BOUNDED_DESIGN_ONLY','last_completed_substep':WORK,'last_result':RESULT,'next_safe_step':NEXT,'production_apply_authorized':False,'ownership_contract_required':True,'overlap_contract_required':True,'rebuild_contract_required':True,'human_authorization_required':True})
    dump(ROADMAP,roadmap)

    master=MASTER.read_text(encoding='utf-8')
    master=sec(master,'## 02 CURRENT MAJOR-LINE POSITION',f'''```text
CURRENT_ERA=ERA56_GLOBAL_INTELLIGENCE_CACHE
ERA56_STATUS=OPEN_BOUNDED_DESIGN_ONLY
CURRENT_STAGE=ERA56_OPENED_BOUNDED_DESIGN_ONLY
LAST_COMPLETED_SUBSTEP={WORK}
ERA55_STATUS=CLOSED_VERIFIED
ERA55_SEAL_PRESERVED=true
PRODUCTION_LEDGER_WRITER_ACTIVE=true
RUNNER_LOCK_ENABLED=true
OPTION_B_DECISION=DEFER_OPTION_B
ERA56_PRODUCTION_APPLY_AUTHORIZED=false
PRODUCTION_MUTATION=false
```''')
    master=sec(master,'## 03 LAST VERIFIED WORK',f'''```text
LAST_COMPLETED={WORK}
LAST_RESULT={RESULT}
LAST_ARTIFACT={rel}
WORK_UNIT_STATUS=CLOSED_VERIFIED
ERA56_OPENED=true
ERA56_MODE=BOUNDED_DESIGN_ONLY
PRODUCTION_MUTATION=false
```

NEXT_SAFE_STEP={NEXT}''')
    master=sec(master,'## 10 NEXT SAFE STEP',f'''```text
NEXT_SAFE_STEP={NEXT}
```

Define one-owner boundaries, overlap prohibitions, immutable snapshot identity, stale detection and deterministic rebuild before any cache implementation.''')
    MASTER.write_text(master,encoding='utf-8')

    hand=HANDOFF.read_text(encoding='utf-8')
    hand=sec(hand,'## 02 CURRENT CONTINUATION CHECKPOINT',f'''PROJECT_STATUS=ACTIVE_ERA56_GLOBAL_INTELLIGENCE_CACHE
CURRENT_ERA=ERA56_GLOBAL_INTELLIGENCE_CACHE
ERA56_STATUS=OPEN_BOUNDED_DESIGN_ONLY
CURRENT_STAGE=ERA56_OPENED_BOUNDED_DESIGN_ONLY
LAST_COMPLETED_SUBSTEP={WORK}
ERA55_STATUS=CLOSED_VERIFIED
ERA55_SEAL_PRESERVED=true
PRODUCTION_LEDGER_WRITER_ACTIVE=true
RUNNER_LOCK_ENABLED=true
ERA56_PRODUCTION_APPLY_AUTHORIZED=false
PRODUCTION_MUTATION=false
CURRENT_HEAD=DYNAMIC_USE_GIT_REV_PARSE_HEAD''')
    hand=sec(hand,'## 03 LAST VERIFIED WORK',f'''LAST_COMPLETED={WORK}
LAST_RESULT={RESULT}
LAST_ARTIFACT={rel}
WORK_UNIT_STATUS=CLOSED_VERIFIED
CURRENT_PROBLEM=ERA56_GLOBAL_CACHE_OWNERSHIP_OVERLAP_REBUILD_CONTRACT_PENDING''')
    hand=sec(hand,'## 07 ALLOWED NEXT DECISIONS',f'''- ERA56 design line: `OPEN`.
- Production cache apply: `BLOCKED`.
- Existing ledger/DB/artifacts remain source authority.
- ERA56 may create only derived, immutable and rebuildable global snapshots.
- Duplicate ledger, duplicate panel bridge and second source of truth are forbidden.

NEXT_SAFE_STEP={NEXT}''')
    HANDOFF.write_text(hand,encoding='utf-8')

    hist=load(HISTORY); events=hist.setdefault('events',[])
    if not any(isinstance(x,dict) and x.get('event_id')=='ERA56_GLOBAL_INTELLIGENCE_CACHE_OPENED' for x in events):
        events.append({'event_id':'ERA56_GLOBAL_INTELLIGENCE_CACHE_OPENED','timestamp_utc':ts,'era':'ERA56','work_unit':WORK,'status':'OPENED_BOUNDED_DESIGN_ONLY','result':RESULT,'artifact':rel,'production_mutation':False,'next_safe_step':NEXT})
    hist['updated_at']=ts; hist['updated_at_utc']=ts; dump(HISTORY,hist)

    marker='## ERA56 GLOBAL INTELLIGENCE CACHE OPENING'
    alm=ALMANAC.read_text(encoding='utf-8')
    if marker not in alm:
        alm=alm.rstrip()+f'''\n\n---\n\n{marker}\n\n- Status: `OPENED_BOUNDED_DESIGN_ONLY`\n- Result: `{RESULT}`\n- ERA55 seal preserved: `true`\n- Production mutation: `false`\n- Cache production apply authorized: `false`\n- Source authority duplicated: `false`\n- Next safe step: `{NEXT}`\n'''
    ALMANAC.write_text(alm,encoding='utf-8')

    TK_AI.write_text(f'''# LATEST TK AI HANDOFF\n\nCURRENT_STATE_AUTHORITY=PROJECT_RUNTIME.json\nSTATE_SYNC_UTC={ts}\nCURRENT_ERA=ERA56_GLOBAL_INTELLIGENCE_CACHE\nERA56_STATUS=OPEN_BOUNDED_DESIGN_ONLY\nLAST_COMPLETED={WORK}\nLAST_RESULT={RESULT}\nERA55_STATUS=CLOSED_VERIFIED\nERA55_SEAL_TAG={SEAL_TAG}\nERA55_SEAL_COMMIT={SEAL_COMMIT}\nERA56_PRODUCTION_APPLY_AUTHORIZED=false\nPRODUCTION_MUTATION=false\nNEXT_SAFE_STEP={NEXT}\n\nERA56 owns only derived immutable rebuildable global snapshots. Existing ledgers, runtime DB and producer artifacts remain authoritative.\n''',encoding='utf-8')

    machine=load(MACHINE); machine['created_at_utc']=ts; machine['collect_mode']='canonical_sync_snapshot_no_tk_machine'; machine['current_state']={'authority':'PROJECT_RUNTIME.json','runtime_status':'ERA56_OPEN_DESIGN_ONLY','active_work_unit':{'id':WORK,'status':'CLOSED_VERIFIED','artifact':rel},'next_safe_step':{'name':NEXT,'status':'READY'},'last_action':{'timestamp':ts,'task':WORK,'result':RESULT,'artifact':rel}}
    machine['known_facts']={'era55_status':'CLOSED_VERIFIED','era55_seal_tag':SEAL_TAG,'era55_seal_commit':SEAL_COMMIT,'era56_opened':True,'era56_mode':'BOUNDED_DESIGN_ONLY','era56_production_apply_authorized':False,'production_mutation':False}
    dump(MACHINE,machine)

    for path in (RUNTIME,ROADMAP,HISTORY,MACHINE,ARTIFACT): load(path)
    if git('rev-list','-n1',SEAL_TAG)!=SEAL_COMMIT: raise RuntimeError('ERA55_SEAL_CHANGED')
    git('add','-A'); check=run(['git','diff','--cached','--check'],check=False)
    if check.returncode!=0:
        print(check.stdout,end=''); print(check.stderr,end=''); raise RuntimeError('DIFF_CHECK_FAILED')
    git('commit','-m',SUBJECT)
    print('ERA56_OPENING=SUCCESS'); print('ERA56_OPENED=true'); print('ERA56_MODE=BOUNDED_DESIGN_ONLY'); print('PRODUCTION_MUTATION=false'); print('ERA56_PRODUCTION_APPLY_AUTHORIZED=false'); print('NEXT_SAFE_STEP='+NEXT); print('LOCAL_COMMIT='+git('rev-parse','HEAD'))
    return 0

if __name__=='__main__': raise SystemExit(main())
