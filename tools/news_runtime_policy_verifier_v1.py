...
def _recent_raw_uids(con: sqlite3.Connection, limit: int) -> List[str]:
    ...
    rows = con.execute(
        "SELECT news_uid FROM news_raw_feed_events ORDER BY " + _q(order_col) + " DESC LIMIT ?",
        [limit]
    ).fetchall()
    return [r[0] for r in rows]

def _derived_counts_for_recent(con: sqlite3.Connection, uids: List[str]) -> Dict[str, int]:
    if not uids:
        return {"news_token_match_events": 0, "news_signal_events": 0, "news_score_events_v1": 0}
    marks = ",".join(["?"] * len(uids))
    return {
        t: con.execute("SELECT COUNT(*) FROM " + _q(t) + " WHERE news_uid IN (" + marks + ")", uids).fetchone()[0]
...
def _duplicate_recent_raw_uids(uids: List[str]) -> List[str]:
    seen = set()
    dup = []
    for uid in uids:
        if uid in seen:
...
def verify_news_runtime_policy(db_path: str = str(DB), recent_limit: int = DEFAULT_RECENT_LIMIT, prior_observation_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if missing:
            ...
            return {
                "decision": "FAIL_NEWS_RUNTIME_POLICY_VERIFIER_V1",
                "failures": failures,
                "warnings": warnings,
        ...
        derived_recent = _derived_counts_for_recent(con, recent_uids)

        uid_violations = _uid_policy_violations(recent_uids, policy["uid_policy"]["allowed_prefixes"])
        recent_duplicates = _duplicate_recent_raw_uids(recent_uids)
        recent_orphans = {
            t: _orphan_recent(con, t, recent_uids)
            for t in ["news_token_match_events","news_signal_events","news_score_events_v1"]
        }
        ...
        result = {
            ...
            "derived_recent_counts": derived_recent,
...
def verify_policy(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        if before["decision"] != "OK_NEWS_RUNTIME_POLICY_VERIFIER_V1":
            return {
                "decision": "BLOCKED_BY_NEWS_RUNTIME_POLICY_VERIFIER_V1",
                "policy_result": before
            }
        result = func(*args, **kwargs)
        after = verify_news_runtime_policy()
        if after["decision"] != "OK_NEWS_RUNTIME_POLICY_VERIFIER_V1":
            return {
                "decision": "POST_RUN_BLOCKED_BY_NEWS_RUNTIME_POLICY_VERIFIER_V1",
...
def main() -> None:
    ...
    args = ap.parse_args()
    print(json.dumps(
        verify_news_runtime_policy(args.db_path, args.recent_limit, args.prior_observation),
        ensure_ascii=False,
        indent=2,
        sort_keys=True
    ))
