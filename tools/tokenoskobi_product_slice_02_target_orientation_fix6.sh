...
TEMP=/root/tokenoskobi_product_slice_02_target_orientation_fix6_patched.sh
COOLDOWN_SEC=75
...
anchor = 'PY\n\nchmod 0700 "$TEMP"'
if s.count(anchor) != 1:
    raise SystemExit("BLOCKED=FIX6_PATCH_ANCHOR_FIX2_END")

patch_block = r'''PY

TEMP="$TEMP" COOLDOWN_SEC="$COOLDOWN_SEC" python3 - <<'PYCOOL'
from pathlib import Path
import os
...
def rep(old, new, label):
    global s
    if s.count(old) != 1:
        raise SystemExit("BLOCKED=FIX6_PATCH_ANCHOR_" + label)
...
s = s.replace(anchor, patch_block, 1)
p.write_text(s, encoding="utf-8")
PY
