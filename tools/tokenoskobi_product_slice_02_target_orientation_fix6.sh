...
rollback(){
  ...
  printf 'TARGET_ORIENTATION_FIX6_RESULT=FAILED\n'
  printf 'FAILED_RC=%s\n' "$rc"
...
BACKUP=/root/tokenoskobi_product_slice_02_orientation_fix6_${STAMP}
mkdir -p "$BACKUP"
cp "$SERVER" "$BACKUP/server.py"
cp "$TEST" "$BACKUP/test.py"
cp "$SERVER" "$BACKUP/server_patched.py"

PATCHED_SERVER="$BACKUP/server_patched.py" python3 - <<'PY'
...
old_import='import json,math,os,re,sqlite3,time,urllib.parse,urllib.request'
new_import='import fcntl,json,math,os,re,sqlite3,time,urllib.error,urllib.parse,urllib.request'
if s.count(old_import)!=1:
    raise SystemExit('BLOCKED=IMPORT_ANCHOR')
...
request_block="""GT_LOCK=Path('/run/tokenoskobi_geckoterminal_rate.lock')
GT_STATE=Path('/run/tokenoskobi_geckoterminal_rate.state')
GT_MIN_INTERVAL_SEC=7.0
GT_MAX_ATTEMPTS=4


def is_geckoterminal(url):
    return urllib.parse.urlsplit(url).netloc.lower()=='api.geckoterminal.com'


def geckoterminal_slot():
    GT_LOCK.parent.mkdir(parents=True,exist_ok=True)
    with GT_LOCK.open('a+',encoding='utf-8') as lock:
        fcntl.flock(lock.fileno(),fcntl.LOCK_EX)
        last=0.0
        try:last=float(GT_STATE.read_text(encoding='utf-8').strip() or 0)
        except Exception:pass
        delay=GT_MIN_INTERVAL_SEC-(time.time()-last)
        if delay>0:time.sleep(delay)
        GT_STATE.write_text(str(time.time()),encoding='utf-8')
...
def request(url,body=None):
    ...
    for attempt in range(1,attempts+1):
        if is_geckoterminal(url):geckoterminal_slot()
        ...
        try:
            ...
            if not (is_geckoterminal(url) and e.code==429 and attempt<attempts):raise
            retry=e.headers.get('Retry-After') if e.headers else None
            try:delay=max(GT_MIN_INTERVAL_SEC,float(retry))
            except Exception:delay=min(60.0,GT_MIN_INTERVAL_SEC*(2**attempt))
            time.sleep(delay)
    raise RuntimeError('GECKOTERMINAL_RETRY_EXHAUSTED')


"""
s=replace_between(s,'def request(url,body=None):\n','def rpc(url,method,params):\n',request_block,'REQUEST')
...
def market(token):
    ...
    except Exception as e:out['errors'].append('TOKEN:'+type(e).__name__+':'+str(e)[:80])
    try:
    ...
    except Exception as e:out['errors'].append('POOLS:'+type(e).__name__+':'+str(e)[:80])
    return out
...
def tech(pool,token):
    ...
    for k,(tf,agg) in specs.items():
        ...
        except Exception as e:out[k]={'status':'VERI_YETERSIZ','error':type(e).__name__+':'+str(e)[:100],'target_token_address':token}
    return out
...
cat > "$BACKUP/test_patched.py" <<'PYTEST'
...
class T(unittest.TestCase):
 ...
 def test_gecko_url_detection(self):self.assertTrue(m.is_geckoterminal('https://api.geckoterminal.com/api/v2/x'));self.assertFalse(m.is_geckoterminal('https://bsc-dataseed.binance.org'))
 ...
 def test_decide_blocks_wrong_target(self):
  market={'selected_pool':{'reserve_usd':100000,'target_token_address':BASE,'orientation_verified':True},'target_orientation_verified':True};tech={'1m':{'status':'OK','target_token_address':QUOTE}}
...
python3 -m py_compile "$BACKUP/server_patched.py" "$BACKUP/test_patched.py"
cp "$BACKUP/server_patched.py" "$SERVER"
cp "$BACKUP/test_patched.py" "$TEST"
chmod 0755 "$SERVER"
chmod 0644 "$TEST"
MODIFIED=1
...
printf 'GECKOTERMINAL_INITIAL_COOLDOWN_SEC=75\n'
sleep 75
...
curl -sS --connect-timeout 5 --max-time 300 -H 'Content-Type: application/json' --data '{"token_address":"0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c"}' "http://127.0.0.1:${SHADOW_PORT}/api/v1/analyze" > "$BACKUP/shadow_wbnb.json"
...
curl -sS --connect-timeout 5 --max-time 300 -H 'Content-Type: application/json' --data '{"token_address":"0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c"}' http://127.0.0.1:8096/api/v1/analyze > "$BACKUP/production_wbnb.json"
...
printf 'GECKOTERMINAL_RATE_LIMIT=CROSS_PROCESS_7_SECONDS\n'
