...
def atomic_json_copy(src: Path, dst: Path) -> None:
    ...
    obj = read_json(src)
    fd, tmp = tempfile.mkstemp(prefix=".news_bridge_", suffix=".json", dir=str(dst.parent))
    try:
...
def validate_authority(obj: Dict[str, Any]) -> list[str]:
    ...
    if isinstance(obj.get("summary"), dict) and isinstance(obj["summary"].get("authority"), dict):
        check_auth(obj["summary"]["authority"], "summary")
    if isinstance(obj.get("hot_gateway"), dict) and isinstance(obj["hot_gateway"].get("authority"), dict):
...
def main() -> int:
    ...
    for name, src in SOURCES.items():
        checks.append({"name": "source_exists:" + name, "ok": src.exists() and src.is_file()})
        if not src.exists():
        ...
        try:
            ...
            if auth_failures:
                failures.extend(["source_authority:" + name + ":" + x for x in auth_failures])
            source_hashes[name] = sha256(src)
        except Exception as e:
            checks.append({"name": "source_parse_ok:" + name, "ok": False, "error": str(e)})
    ...
    summary = source_objs.get("news_coverage_readmodel_consumer_summary_v1.json", {})
    ...
    watch = source_objs.get("news_producer_health_watch_and_hot_gateway_review_v1.json", {})
    ...
    if not failures:
        ...
        tmp_manifest = ACTIVE_DATA / "news_active_panel_data_bridge_manifest_v1.json"
        fd, tmp = tempfile.mkstemp(prefix=".news_bridge_manifest_", suffix=".json", dir=str(ACTIVE_DATA))
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as f:
    ...
    hash_match = {name: target_hashes_after.get(name) == source_hashes.get(name) for name in SOURCES.keys()}
    ...
    status = {
        ...
        "decision": "OK_NEWS_ACTIVE_PANEL_DATA_BRIDGE_APPLIED" if not failures else "FAIL_NEWS_ACTIVE_PANEL_DATA_BRIDGE",
        "source_dir": str(ROOT / "runtime/state"),
        "target_dir": str(ACTIVE_DATA),
        "files": sorted(SOURCES.keys()),
        "source_hashes": source_hashes,
