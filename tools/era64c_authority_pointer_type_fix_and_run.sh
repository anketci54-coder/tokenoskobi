...
TARGET="/tmp/era64c_real_historical_wallet_replay_and_validation_fixed_v5.sh"
...
if old_authority not in text:
    raise RuntimeError("ERA64C_AUTHORITY_BLOCK_PATTERN_MISSING_V2")
text = text.replace(old_authority, new_authority, 1)

target.write_text(text, encoding="utf-8")
target.chmod(0o700)
print("ERA64C_AUTHORITY_POINTER_TYPE_FIX_V2=VERIFIED")
PY
