...
## Validator exact-term compatibility block

Bu bölüm içerik doğrulama terimlerini birebir taşır; yeni kural eklemez, Contract3 içeriğini netleştirir.

- Veriye göre konuş, veri yoksa konuşma.
- source_uid yoksa motion yok.
- No source_uid means no motion.
- Fake/demo/mock veri production motion tetikleyemez.
- Whale Radar sadece fresh whale event ile hareket eder.
- Klinik heartbeat sadece clinical_watch ile çalışır.
- Morg cold breath sadece morg_status ile çalışır.
- TRUTH_PASS için source_uid, source_layer, reason, freshness ve expiry zorunludur.
- PRODUCT_PASS için hareket premium, anlamlı, veriye bağlı ve dikkat dağıtmayan seviyede olmalıdır.
- motion_state_layer hareketin tek sahibidir.
