...
```text
LAST_COMPLETED=PRE_ERA57_LIVE_RAW_RUNNER_RESOLUTION_AND_RUNTIME_ENTRY_DECISION
LAST_RESULT=WARN_RAW_RUNNER_UNRESOLVED_REPAIR_REVIEW_REQUIRED
LAST_ARTIFACT=data/control/pre_era57_live_raw_runner_resolution_and_runtime_entry_decision_v1.json
RAW_RUNNER_RESOLVED=false
RAW_NATURAL_CYCLE_OK=false
DECISION=AUTHORIZE_BOUNDED_RAW_PATH_REPAIR_REVIEW
ERA57_OPENED=false
PRODUCTION_MUTATION=false
```

NEXT_SAFE_STEP=PRE_ERA57_RAW_RUNNER_BOUNDED_PATH_REPAIR_DECISION
...
Follow the recorded raw-runner decision. ERA57 remains closed until explicit opening approval.
